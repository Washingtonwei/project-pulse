...
- **FR IDs are independent of section numbering.** SRS §4.2 FRs (`FR-SAVE-1`, `FR-LOCK-3`, etc.) use the `FR-<AREA>-<n>` format (parallel to `UC-<AREA>-<n>`): FR-SAVE-* autosave, FR-LOCK-* locking, FR-COL-* collaboration, FR-VAL-* validation, FR-AI-* AI, FR-TPL-* templates, FR-GLO-* glossary, FR-HIS-* history & authorship metadata, FR-SEC-* security, FR-EXP-* export, FR-IMP-* import, FR-PERF-* performance, FR-NOT-* notifications. §4.2 is for non-use-case behaviors only — a use case is itself a high-level FR (its steps are its spec), so do not expect or add a separate FR that merely restates a use-case CRUD flow. Do NOT renumber FR IDs as part of section renumbering — they're a separate identifier space, append-only within each area.
