...
- **Architecture deltas (Level 1)** — the RAM module architecture-of-record (`docs/ram/design/architectural-design.md`) draws the component boundaries and subsystem owners *provisionally*, ahead of any area being designed. If this design contradicts that provisional shape — a component that should split or merge differently than drawn, a cross-cutting subsystem this area actually owns or plugs into differently, a genuinely new `ram/*` component — **say so here with the proposed edit to that doc**, so the Level-1 change is reviewed and approved *together with* this design rather than drifting. (A platform-wide delta — `docs/pulse-core/design/architectural-design.md` — is flagged for *separate* confirmation, not bundled; see Phase 3.)
...
- **Revise the RAM module Level-1 doc when this area contradicts it.** The RAM module architecture-of-record (`docs/ram/design/architectural-design.md`) holds *provisional* component boundaries and subsystem placements drawn before any area was designed; the first real design of an area is exactly when they get validated. So if the approved design changed that shape — a component boundary, a subsystem owner, a new `ram/*` component, a cross-area model adjustment within RAM — **fold it into that doc as part of this run** (the component diagram, the subsystems table, the affected prose), then record it in Phase 4. This Level-1 edit must already have been surfaced and approved in the Phase 2 proposal (see *Architecture deltas*) — never make a silent Level-1 change. Because that doc lives under `docs/ram/`, follow `docs/ram/CLAUDE.md` and run `/build` after.
- **The platform Level-1 doc is different — confirm first.** A change to the *host* architecture (containers, conventions, deployment) lives in `docs/pulse-core/design/architectural-design.md`, which **both** modules inherit, so it's higher-stakes. **Flag it and confirm** before touching it; it is never rewritten by a single `/design` run.
...
If this run **revised the RAM module architecture-of-record** (per Phase 3), that edit is part of this design — confirm it's saved, run `/build` to resync `docs/ram/` anchors and cross-references, and note the Level-1 change in the handoff so the developer knows the module architecture moved.
