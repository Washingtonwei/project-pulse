...
argument-hint: <UC-ID>  (e.g. /design UC-DOC-create-use-case)
---
...
1. Read the target use case in `docs/requirements/use-cases.md` (locate it with Grep on `### **UC-` and the area-prefixed ID, e.g. `UC-DOC-create-use-case` — never read that file whole; use cases are H3 headings grouped under H2 area headings). Capture: primary/secondary actors, trigger, preconditions, postconditions, main success scenario, extensions, business rules, associated information, related use cases.
2. Resolve its **functional requirements**: the use case's own "The system …" steps + Associated Information **are** its detailed functional spec and primary acceptance criteria (a use case is a high-level FR). Then add the SRS's cross-cutting, system-level non-use-case FRs it depends on (`FR-<AREA>-<slug>` format: `FR-SAVE-*` autosave, `FR-VAL-*` validation, `FR-AI-*` AI, `FR-TPL-*` templates, `FR-GLO-*` glossary, `FR-HIS-*` history & authorship metadata, `FR-SEC-*` security, `FR-NOT-*` notifications) — the traceability matrix lists these per use case. (Behavior a use case fully captures has no FR: section locking is governed by `BR-edit-lock-required`/`BR-lock-expiry` + the editing UC steps; real-time collaboration by UC-COL-collaborative-edit; export/import by their UCs + the `SI-*` interface requirements.)
...
- **What this realizes** — the `UC-<AREA>-<slug>` and the cross-cutting `FR-<AREA>-<slug>` IDs it builds on. **Flag any use-case step that relies on cross-cutting behavior with no backing non-use-case FR** (a spec gap to confirm or fill) and any cited FR that doesn't exist.
...
Update the use case's row in `docs/traceability.md`: set **Build** to `📐 Designed` (the design-of-record exists at `design/<area>.md`; not yet coded) and leave **Verify** as `—` (nothing built to verify yet), fill the **Design** column with that doc (`design/<area>.md`, optionally anchored to the use case's subsection), and make sure the `FR IDs` column lists the cross-cutting FRs the design depends on. Leave the Frontend / Backend / Tests columns as `—` — `/implement` fills those when it builds.

**The problem-validated beat.** If a run **firms the problem but stops before an approved solution design** — the challenge loop surfaced spec fixes that need their own review round, or the developer pauses after Phase 2 — set **Build** to `🔬 Problem-validated` instead (Design stays `—`, Verify `—`), so the firmed-but-undesigned state is visible and the challenge event is on the record (its evidence is the spec diff the loop produced). A run that continues on to an approved design records `📐 Designed` directly; `🔬` need not be written as an intermediate step when the same run reaches `📐`.
