...
4. Resolve **vocabulary**: look up the domain terms in `docs/ram/requirements/project-glossary.md`. Use the defined terms in code identifiers and UI strings; do not introduce synonyms.
5. Check **`docs/ram/traceability.md`** for the existing row: is any of this already built? Reuse it; don't duplicate.
...
4. **Keep the design-of-record honest.** If implementation forced a change from the approved design (a different class boundary, an extra table, a revised flow), update `docs/ram/design/<area>.md` to match — revise the area-wide class/ER diagram in place, adjust the affected sequence. The design doc must describe what was actually built.
