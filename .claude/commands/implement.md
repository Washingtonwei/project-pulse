...
3. Resolve its **functional requirements**: the use case's own "The system …" steps + Associated Information **are** its detailed functional spec and primary acceptance criteria (a use case is a high-level FR). Add the SRS's cross-cutting non-use-case FRs it depends on (`FR-<AREA>-<n>`: `FR-SAVE-*`, `FR-LOCK-*`, `FR-COL-*`, `FR-VAL-*`, `FR-AI-*`, `FR-TPL-*`, `FR-GLO-*`, `FR-HIS-*`, `FR-SEC-*`, `FR-EXP-*`, `FR-IMP-*`, `FR-NOT-*`) — the traceability matrix lists these per use case.
