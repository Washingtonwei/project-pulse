...
- **ID reference resolution — every ID space, not just FR/BR.** Each ID an artifact defines is a handle other artifacts point at: a reference resolving to no definition is a dangling pointer; two definitions of one ID is a collision. For **every** ID space, verify references resolve and definitions are unique. Every ID is now a **name-based slug**, not a number, so there is no contiguous-`1..max` invariant to check — a slug's identity is its name, and inserting or reordering renumbers nothing:
  ...
  - **`BR-<slug>`** — defined in `business-rules.md`; referenced in use cases' **Business Rules** field and where the SRS cites a rule. Flag dangling refs and **orphan rules** (a `BR-*` no UC or SRS cites — report only; a rule may legitimately be system-wide).
...
- **Traceability completeness — forward and backward, not just row existence.** Beyond every UC having a row, verify the requirement chain is closed both ways:
  - **Forward (no orphan scope):** every `vision-and-scope.md` feature and every `BO-*` is realized by at least one use case or FR. UC realizers live in `traceability.md`'s `Supports BOs` column (one row per UC); FR realizers carry an inline `_(Supports BO-…)_` annotation at the end of the FR statement (see `FR-NOT-weekly-reminder` for the canonical pattern). A feature nothing implements is unspecified promised scope — flag it. **Exempt BOs explicitly marked as platform-level emergent objectives** — a BO whose definition ends with the parenthetical `*(Platform-level emergent objective; …)*` (currently `BO-RAM-research`) has no direct UC/FR realizer by design and is not a coverage gap; report it separately as "platform-level (no realizer, as intended)."
...
- **IDs are independent of headings and are name-based slugs (never renumbered).** SRS Non-Use Case Functional Requirements use the `FR-<AREA>-<slug>` format (parallel to `UC-<AREA>-<slug>`): FR-SAVE-* autosave, FR-LOCK-* locking, FR-COL-* collaboration, FR-VAL-* validation, FR-AI-* AI, FR-TPL-* templates, FR-GLO-* glossary, FR-HIS-* history & authorship metadata, FR-SEC-* security, FR-EXP-* export, FR-IMP-* import, FR-NOT-* notifications. Business rules use `BR-<slug>`. A use case is itself a high-level FR (its steps are its spec), so do not expect or add a separate FR that merely restates a use-case CRUD flow. None of these IDs are section numbers, and each handle is a name not a number — never strip them or treat them as a sequence.
