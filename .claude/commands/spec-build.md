...
- **ReqLint writing quality** — apply to `FR-*` "shall" statements in the SRS:
...
- `docs/requirements/use-cases.md` is large (~275 KB — it now holds the platform-core areas plus the requirements-authoring areas). Use Grep to locate headings and references — only Read targeted sections when you must inspect content. Never read it whole.
