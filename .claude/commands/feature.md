...
2. Resolve its **functional requirements**: the use case's own "The system …" steps + Associated Information **are** its detailed functional spec and primary acceptance criteria (a use case is a high-level FR). Then add the SRS §4.2 cross-cutting system-level FRs it depends on (`FR-<AREA>-<n>` format: `FR-SAVE-*` autosave, `FR-LOCK-*` locking, `FR-COL-*` collaboration, `FR-VAL-*` validation, `FR-AI-*` AI, `FR-TPL-*` templates, `FR-GLO-*` glossary, `FR-HIS-*` history & authorship metadata, `FR-SEC-*` security, `FR-EXP-*` export, `FR-IMP-*` import, `FR-PERF-*` performance, `FR-NOT-*` notifications) — the traceability matrix lists these per use case.
...
6. Read the **sections that constrain the design**, not just the use case: the Architectural Design doc (`docs/ram/design/architectural-design.md`) — its Container Diagram and Design & Implementation Constraints (fit the *unified* Project Pulse architecture — don't invent a parallel one) and its Operating Environment / Assumptions & Dependencies — plus SRS §6.1 Business Domain Model (your entities/tables must align to it), SRS §8 Quality Attributes (performance/security/availability are non-functional acceptance criteria even when no UC step names them), and SRS §7.2 Software Interfaces / §7.3 API Document for AI/CFG/EXP areas (the LLM service). Treat the Design & Implementation Constraints and SRS §8 as binding — but skeptically (a constraint can be stale or wrong; if so, flag it, don't just obey it).
