...
6. Read the **sections that constrain the design**, not just the use case: the Architectural Design doc (`docs/ram/design/architectural-design.md`) — its Container Diagram and Design & Implementation Constraints (fit the *unified* Project Pulse architecture — don't invent a parallel one) and its Operating Environment / Assumptions & Dependencies — plus the SRS's Business Domain Model (your entities/tables must align to it), the SRS's Quality Attributes (performance/security/availability are non-functional acceptance criteria even when no UC step names them), and the SRS's Software Interfaces / API Document for AI/CFG/EXP areas (the LLM service). Treat the Design & Implementation Constraints and the SRS's Quality Attributes as binding — but skeptically (a constraint can be stale or wrong; if so, flag it, don't just obey it).
