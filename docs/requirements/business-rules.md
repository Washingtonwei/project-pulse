...
- **BR-team-assignment-required:** Only a student authors a weekly activity report or a peer evaluation, and only once she is assigned to a team. A student enrolls in a course section before she is assigned to a team, so until that assignment she has no team whose work she could report on and no teammates she could evaluate, and she may neither submit nor read either record. An instructor or course admin does not author them at all; she reads the records of the course sections she is assigned to or owns, per BR-section-scoped-access. A submission that fails either condition is rejected by naming the unmet condition, not as a permission refusal.
