...
- **Read-only.** A critique neither locks nor modifies anything. Acting on a finding goes through the normal authoring path — a manual edit (UC-DOC-2 / UC-DOC-6) or an accepted rewrite via UC-AI-8 — never a direct write from the assistant.
...
- **Enablement** — the critique assistant runs only where it is enabled for the course section (FR-AI-7, UC-CFG-2).
- **Never author without acceptance** — the assistant proposes; content changes only through the student's explicit, per-item acceptance (FR-AI-3, FR-AI-8, BR-17). No silent writes, no "accept all".
