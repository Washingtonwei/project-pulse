...
This directory holds the **spec→design→trace chain** for Project Pulse — the durable, version-controlled documentation that drives the implementation. It is organized **doctype-first**: one set of docs for the whole product, covering the original features — the shared foundation (courses/sections/teams) and performance tracking (weekly activity reports, peer evaluations) — and the RAM (Requirements Authoring & Management) module.
