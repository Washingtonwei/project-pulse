...
- Actuator + Micrometer expose JVM / HTTP / application metrics, visualized in **Grafana** via **Prometheus** (both `docker-compose`, local dev only). Note that `/actuator/prometheus` is **not currently a registered endpoint** despite `micrometer-registry-prometheus` being a dependency, observed 2026-09-06 from the `/actuator` discovery listing, which shows only `beans`, `configprops`, `env`, `health`, `info`, `loggers`, `mappings`, and `metrics`. `heapdump`, `httpexchanges`, `prometheus`, and `custom-beans` appear in the dev exposure list but resolve to nothing in this build (Spring Boot 4 splits several of these into modules that are not on the classpath; `httpexchanges` additionally needs an `HttpExchangeRepository` bean that is never defined). So the local Prometheus scrape has been collecting nothing.
...
- **CONFIRMED INCIDENT, 2026-09-06 ([TD-1](#risks-and-technical-debt), P0):** `/actuator/**` was publicly exposed and unauthenticated, and secrets were verified to have been readable.** The actuator paths fall outside the `/api/v1` rules and hit `anyRequest().permitAll()`, and the shared exposure list in `application.yml` applied to prod and staging as well as dev, with `show-values: always` disabling Boot's masking on `env` and `configprops`. **Verified by unauthenticated request on 2026-09-06** against both the production host and the staging slot: `GET /actuator/env` returned `200` with content type `application/vnd.spring-boot.actuator.v3+json` and `activeProfiles: ["prod"]`, and a masked-value count of **zero** on both. This was a live disclosure, not an inferred risk. **What was readable** (categories only; no values are recorded here, and none should be): the database host, username and password; the mail account and its app password; the Azure App Service authentication signing and encryption keys; the managed-identity secret and header; and the subscription, resource-group, Key Vault and site identifiers, alongside the full JVM and OS environment. **Remediation has three parts.** (1) *Code*, landed on 2026-09-06: exposure profiled to a safe baseline with the full surface moved to dev, `EndpointRequest` rules making health and info anonymous and everything else `ROLE_admin`, and `/actuator/info` trimmed. (2) *Immediate mitigation*, no deploy required, since environment variables override the packaged YAML: set `MANAGEMENT_ENDPOINTS_WEB_EXPOSURE_INCLUDE=health,info` and `MANAGEMENT_ENDPOINT_ENV_SHOWVALUES=never` as App Settings on **both** slots, marked as deployment-slot settings so a swap cannot move them. (3) *Credential rotation*, which is the part that actually closes the incident, **completed 2026-09-06**: the database admin password was reset and re-secreted, the Google account password was changed (which invalidates prior app passwords) and a new app password issued, and the restarts rotated the managed-identity and App Service auth values implicitly. Verified by a cold-start health check returning `UP`, which is conclusive here because the mail contributor authenticates a fresh SMTP session on every check, so it could not pass while the app still held the invalidated app password. The residual network exposure that made the disclosed credentials reachable in practice is split out as [TD-12](#risks-and-technical-debt). **Two observations worth keeping.** Production was running an older jar (1.5.0) whose packaged config still carried the full exposure list, so the deployed surface did not match the source tree. And production serves `index.html` with `200 text/html` for unknown paths, so a `200` on an actuator path there is not by itself evidence that the endpoint exists: `/actuator/heapdump` and `/actuator/httpexchanges` answered `200` on prod but `404` on staging, and the content type is what distinguishes a real endpoint from the SPA fallback.
...
| QS-7 | Performance | A student loads a team's requirements graph and runs ReqLint at course scale (~75 total users, ≤ 100 concurrent editors — SCA-cohort-load; ~1,000 artifacts) · normal op | Page and validation respond within target | ReqLint returns within 3 s for 95% of runs on a single document (PER-validation-speed); p95 graph-load API response < 500 ms at ~1,000 artifacts (PER-graph-load) |
