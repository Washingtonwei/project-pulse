...
|---|---|---|---|---|---|
| TD-1 | **P0** | Debt (config) → **incident** | **Confirmed live 2026-09-06**, both prod and staging: `/actuator/env` returned unmasked values to an anonymous caller, disclosing database and mail credentials, Azure App Service auth keys, managed-identity material and subscription/Key Vault identifiers. Prod was running an older jar (1.5.0) carrying the pre-fix config | **Code fix landed** (commit `bb47bf0`): safe exposure baseline, `EndpointRequest` rules (health/info anonymous, rest `ROLE_admin`), `info` trimmed, covered by `ActuatorSecurityIntegrationTest`. **Outstanding: credential rotation** (database password, mail app password; restart rotates the managed-identity values) and the App Settings stopgap on both slots until the fix deploys. Keep P0 until rotation is confirmed | Observability, Security |
