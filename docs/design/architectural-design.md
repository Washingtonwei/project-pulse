...
- **RESOLVED 2026-09-06 ([TD-1](#risks-and-technical-debt), was P0) — `/actuator/**` was publicly exposed and unauthenticated.** It falls outside the `/api/v1` rules and hit `anyRequest().permitAll()`, and the shared exposure list applied to prod and staging as well as dev. With `show-values: always` on `env` and `configprops`, Boot's masking was off, so **resolved secrets (DB credentials, Key-Vault values) were readable as plaintext by anonymous callers in production**, alongside `beans`, `mappings`, `metrics`, `health` details, and a runtime-writable `loggers`. Two changes close it. **(1) Exposure is now profiled:** `application.yml` carries the safe baseline (`health`, `info`, `show-details: when-authorized` with `roles: admin`, no `show-values` overrides) and `application-dev.yml` opens the rest, so a deployed profile that overrides nothing inherits the safe set. **(2) Two rules in the filter chain:** `EndpointRequest.to(HealthEndpoint, InfoEndpoint)` is `permitAll` because the platform probe cannot authenticate and, with `when-authorized`, discloses nothing; `EndpointRequest.toAnyEndpoint()` requires `ROLE_admin`. `EndpointRequest` resolves the configured actuator base path rather than hardcoding `/actuator/**`. Covered by `ActuatorSecurityIntegrationTest`. **(3) `/actuator/info` is trimmed in the baseline**, since it is anonymous and is now the only exposed endpoint besides health: git drops from `full` to `simple` (branch, commit id, commit time) so the whole of `git.properties` is no longer published, including committer and build-user names and emails, the remote URL and the build host; and the `java` and `os` blocks are disabled, since an exact JVM and OS version narrows CVE selection for an anonymous caller. Dev restores all three. **Note on the original wording:** the `heapdump` exposure claimed here was never reachable in this build, since that endpoint is not registered (see the bullet above); the credential disclosure via `env`/`configprops` was real. **Residual:** `ROLE_admin` is the course-administrator role, so it conflates "runs a course" with "may read configuration"; a dedicated ops role is the cleaner end state. Binding management to a private port, the strongest option, is not available on Azure App Service, which routes a single port per container.
...
|---|---|---|---|---|---|
| TD-1 | ~~**P0**~~ | Debt (config) | **Closed 2026-09-06.** `/actuator/**` was public & unauthenticated; `env`/`configprops` with `show-values: always` returned resolved secrets (DB creds, Key-Vault values) as plaintext in production. (`heapdump` was listed but is not a registered endpoint in this build.) | Done: exposure profiled to a safe baseline in `application.yml` with the full surface moved to dev, plus `EndpointRequest` rules making health/info anonymous and every other endpoint `ROLE_admin`. Residual: `ROLE_admin` is the course-admin role, not an ops role | Observability, Security |
