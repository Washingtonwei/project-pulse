...
The artifact types named here are the canonical `RequirementArtifactType` set reconciled in OI-15 (glossary ↔ SRS §6.1 enum); Stakeholder and Dependency are tracked artifact types. User stories are a deferred type (see SRS §6.1); a User Story, when present, is a Tier 3 behavioral artifact.
